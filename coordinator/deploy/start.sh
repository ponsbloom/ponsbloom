...
# Ponsbloom Cloud persistent storage (survives upgrades via blue-green disk transfer).
...
if [ ! -d "/data/step-ca/config" ]; then
    ...
    echo "ponsbloom-step-ca" > /data/step-ca/secrets/password
...
if [ -n "$MICROMDM_API_KEY" ]; then
    ...
    if [ -n "$MDM_PUSH_P12_B64" ] && [ ! -f /data/micromdm/push.crt ]; then
        ...
        openssl pkcs12 -in /tmp/push.p12 -clcerts -nokeys -passin pass:ponsbloom \
            -out /data/micromdm/push.crt 2>/dev/null
        openssl pkcs12 -in /tmp/push.p12 -nocerts -nodes -passin pass:ponsbloom \
    ...
    micromdm serve \
        ...
        -api-key "${MICROMDM_API_KEY:-ponsbloom-micromdm-api}" \
    ...
    if [ -f /data/micromdm/push.crt ] && [ ! -f /data/micromdm/.push_imported ]; then
        ...
        mdmctl config set \
            ...
            -api-token "${MICROMDM_API_KEY:-ponsbloom-micromdm-api}" \
            -skip-verify
...
# ---- Coordinator (PID 1 — receives SIGTERM from Ponsbloom Cloud) ----
