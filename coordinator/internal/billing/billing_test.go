...
func TestReferrerStoreLifecycle(t *testing.T) {
	...
	if err := st.CreateReferrer("account-1", "PONS-ABC123"); err != nil {
		t.Fatalf("create: %v", err)
	}

	ref, err := st.GetReferrerByCode("PONS-ABC123")
	if err != nil {
	...
	if err := st.CreateReferrer("account-2", "PONS-ABC123"); err == nil {
		t.Fatal("expected error on duplicate code")
	}
	if err := st.CreateReferrer("account-1", "PONS-XYZ789"); err == nil {
