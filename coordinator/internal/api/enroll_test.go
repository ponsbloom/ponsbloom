...
func TestGenerateCombinedProfile(t *testing.T) {
	...
	expectedURLs := []string{
		...
		"https://api.darkbloom.dev/acme/ponsbloom-acme/directory",
	}
	...
	if !strings.Contains(profile, "io.ponsbloom.enroll.acme."+serial) {
		t.Error("profile missing ACME payload identifier with serial")
	}
	if !strings.Contains(profile, "io.ponsbloom.enroll."+serial) {
...
func TestGenerateCombinedProfileDifferentDomains(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.Contains(profile, tt.baseURL+"/acme/ponsbloom-acme/directory") {
