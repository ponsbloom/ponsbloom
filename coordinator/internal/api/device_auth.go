...
func (s *Server) handleDeviceToken(w http.ResponseWriter, r *http.Request) {
	...
	case "approved":
		...
		rawToken := "ponsbloom-pt-" + hex.EncodeToString(tokenBytes)
...
func (s *Server) handleDeviceApprove(w http.ResponseWriter, r *http.Request) {
	...
	if time.Now().After(dc.ExpiresAt) {
		writeJSON(w, http.StatusGone, errorResponse("expired_code", "this code has expired — run 'ponsbloom login' again"))
		return
	}
