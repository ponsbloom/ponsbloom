# Ponsbloom - Decentralized Private Inference

Ponsbloom is a decentralized/private inference stack for Apple Silicon Macs. Consumers use OpenAI-compatible APIs, the coordinator handles routing/auth/billing/attestation, and providers run local text, transcription, and image workloads on macOS hardware.
...
│   └── main.go       verifies attestation blobs from /tmp/ponsbloom_attestation.json
└── internal/
...
app/Ponsbloom/            SwiftUI macOS menu bar app
├── Sources/Ponsbloom/
│   ├── PonsbloomApp.swift
...
├── Sources/PonsbloomEnclave/      enclave key + attestation library + FFI bridge
├── Sources/PonsbloomEnclaveCLI/   `ponsbloom-enclave` CLI (attest, sign, info)
...
├── bundle-app.sh     build Ponsbloom.app + DMG
...
- The macOS app is a real operational client, not just a wrapper. It manages installation, onboarding, launchd integration, diagnostics, and subprocess supervision for `ponsbloom`.
...
- Prod coordinator runs on Ponsbloom Cloud (TEE) as app `d-inference` at `api.darkbloom.dev`. Build target: `coordinator/Dockerfile`. Dev coordinator runs on Google Cloud (see `docs/dev-environment.md`).
...
Quick coordinator deploy (prod, Ponsbloom Cloud):

```bash
# Ponsbloom Cloud builds from the repo via coordinator/Dockerfile and blue-green deploys.
...
- Provider image serving is opt-in through `PONSBLOOMENCE_IMAGE_MODEL` and `PONSBLOOMENCE_IMAGE_MODEL_PATH`; if you touch image flows, verify both the coordinator catalog and provider env/config path handling.
