...
#
# The admin token is stored at ~/.ponsbloom/admin_token and reused until it expires.

COORDINATOR_URL="${PONSBLOOMENCE_COORDINATOR_URL:-https://api.darkbloom.dev}"
TOKEN_FILE="$HOME/.ponsbloom/admin_token"

# ─── Auth helpers ───────────────────────────────────────────

get_token() {
    # Check for admin key (dev/pre-prod).
    if [ -n "${PONSBLOOMENCE_ADMIN_KEY:-}" ]; then
        echo "$PONSBLOOMENCE_ADMIN_KEY"
        return
    fi
...
case "${1:-help}" in
    ...
    help|--help|-h)
        ...
        echo ""
        echo "Environment:"
        echo "  PONSBLOOMENCE_COORDINATOR_URL   Coordinator URL (default: https://api.darkbloom.dev)"
        echo "  PONSBLOOMENCE_ADMIN_KEY         Admin key (pre-prod shortcut, skips Privy login)"
        ;;
    *)
