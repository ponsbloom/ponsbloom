# Ponsbloom Privacy Policy

Updated: April 17, 2026

This Privacy Policy explains how the Ponsbloom contributors, operating the Ponsbloom platform ("Ponsbloom," "Ponsbloom," "we," "us," or "our") collects, uses, discloses, and otherwise processes personal information in connection with Ponsbloom's websites, console, APIs, software, provider applications, hosted services, and related products and features (collectively, the "Services").

**Standalone Document.** This Privacy Policy is a standalone document governing the Ponsbloom Services only. It is separate from and does not incorporate the Ponsbloom Privacy Policy or other privacy notices governing other Ponsbloom affiliate products. Your use of those products is governed solely by their respective privacy notices. This Privacy Policy is designed for the Ponsbloom product, including consumer-facing API and console features and provider-facing node software. It does not apply to third-party services we do not control, even if they interoperate with the Services.
...
Further, provider Macs enrolled in the platform are subject to periodic automated SecurityInfo queries sent via Apple's MDM protocol through Apple's Push Notification service (APNs). These queries request SIP status, Secure Boot level, and Authenticated Root Volume integrity from the device's OS MDM client and not from Ponsbloom software. This querying occurs on an ongoing basis throughout the provider's participation in the platform.

MDM enrollment is configured for security verification. Ponsbloom requests only the minimum MDM permissions necessary to verify your device's security posture, specifically the ability to query device information and security status. We have deliberately not requested MDM permissions that would allow us to erase, lock, install or remove applications, or access user content or application data.
...
Because Ponsbloom is an inference platform, Content handling is central to how the Services work. The following disclosures are intended to be precise and conservative.
...
To submit a privacy request, contact us at notices@ponsbloom.org.
...
For questions or requests about this Privacy Policy, contact: notices@ponsbloom.org
