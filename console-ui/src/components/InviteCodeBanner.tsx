...
const DISMISSED_KEY = "ponsbloom_invite_dismissed";

export function InviteCodeBanner() {
  // Start hidden on both server and first client render, then reveal in an
  ...
  useEffect(() => {
    if (localStorage.getItem(DISMISSED_KEY) !== "1") setDismissed(false);
  }, []);
